...
$(document).ready(function(){  
	...
	let _토지담보제공면적 = PRT_22_토지평가가격등계산.getData() == '' ? 0 : PRT_22_토지평가가격등계산.getData()[0].토지담보제공면적;

	lowest_tot.최종평가_금액 = Number(PRT_22_토지평가가격등계산.getData() == null || PRT_22_토지평가가격등계산.getData() == '' ? 0 : fn_input_number_decrypt(PRT_22_토지평가가격등계산.getData()[0].최종평가_금액))  + Number(PRT_21_건물평가가격등계산.getData() == null || PRT_21_건물평가가격등계산.getData() =='' ? 0 : fn_input_number_decrypt(PRT_21_건물평가가격등계산.getData()[0].최종평가_금액));
	...
	lowest_tot.최종평가_단가 = (lowest_tot.최종평가_금액 / _토지담보제공면적).toFixed(0);
	lowest_tot.최종평가_단가_평 = (lowest_tot.최종평가_단가 * 3.3058).toFixed(0);

	lowest_tot.최종평가_금액 = fn_input_number(lowest_tot.최종평가_금액);
	lowest_tot.최종평가_단가 = fn_input_number(lowest_tot.최종평가_단가);
	lowest_tot.최종평가_단가_평 = fn_input_number(lowest_tot.최종평가_단가_평);

	lowest_tot.예상낙찰가_금액 = Number(PRT_22_토지평가가격등계산.getData() == null || PRT_22_토지평가가격등계산.getData() == '' ? 0 : fn_input_number_decrypt(PRT_22_토지평가가격등계산.getData()[0].예상낙찰가_금액))  + Number(PRT_21_건물평가가격등계산.getData() == null || PRT_21_건물평가가격등계산.getData() =='' ? 0 : fn_input_number_decrypt(PRT_21_건물평가가격등계산.getData()[0].예상낙찰가_금액));
	...
	lowest_tot.예상낙찰가_단가 = (lowest_tot.예상낙찰가_금액 / _토지담보제공면적).toFixed(0);
	lowest_tot.예상낙찰가_단가_평 = (lowest_tot.예상낙찰가_단가 * 3.3058).toFixed(0);

	lowest_tot.예상낙찰가_금액 = fn_input_number(lowest_tot.예상낙찰가_금액);
	lowest_tot.예상낙찰가_단가 = fn_input_number(lowest_tot.예상낙찰가_단가);
	lowest_tot.예상낙찰가_단가_평 = fn_input_number(lowest_tot.예상낙찰가_단가_평);
	...
	$('#prt_22_lowest').html(
		Mustache.render($('#prt_22_lowest_template').html(), {'prt22': PRT_22_토지평가가격등계산.getData(), 'prt21':PRT_21_건물평가가격등계산.getData() ,'list_tot':lowest_tot} )
	);
});