...
function MC_002_초기화_Query_Loading() {
    ...
    if (isNotEmpty(GetValue('감정순번'))) {
        ...
        GetZoonData('Q_20_입력표_낙찰가율보정표').GetRow(0, [
            {key: 'Combo_보정표_상가의접근성', value: '상업용_접근성'},
            {key: 'Combo_보정표_상가의층별위치', value: '상업용_층별위치'},
            {key: 'Combo_보정표_상권의발달정도', value: '상업용_발달정도'},
            {key: 'Combo_보정표_건축물경과년도', value: '상업용_경과년도'},
            {key: 'Combo_보정표_점유자의권리형태', value: '점유자의권리형태'},
            {key: 'MkEdit_보정표_상가의접근성_적용율', value: '상업용_접근성_적용률'},
            {key: 'MkEdit_보정표_상가의층별위치_적용율', value: '상업용_층별위치_적용률'},
            {key: 'MkEdit_보정표_상권의발달정도_적용율', value: '상업용_발달정도_적용률'},
            {key: 'MkEdit_보정표_건축물의경과년도_적용율', value: '상업용_경과년도_적용률'},
        ...
        GetZoonData('Q_감정가_09_감정평가').GetRow(0, [
            {key: 'MkEdit_평가_비준_최저_총액', value: '비준_최저가격'},
            {key: 'MkEdit_평가_비준_최저_단가', value: '비준_최저가격_단가'},
            {key: 'MkEdit_평가_비준_최고_총액', value: '비준_최고가격'},
            {key: 'MkEdit_평가_비준_최고_단가', value: '비준_최고가격_단가'},
            {key: 'MkEdit_평가_비준_비율', value: '비준_비율'},
            {key: 'MkEdit_평가_수익_최저_총액', value: '수익_최저가격'},
            {key: 'MkEdit_평가_수익_최저_단가', value: '수익_최저가격_단가'},
            {key: 'MkEdit_평가_수익_최고_총액', value: '수익_최고가격'},
            {key: 'MkEdit_평가_수익_최고_단가', value: '수익_최고가격_단가'},
            {key: 'MkEdit_평가_수익_비율', value: '수익_비율'},
            ...
            {key: 'MkEdit_평가_격차율_최저', value: '결정가격_격차율_최저'},
            {key: 'MkEdit_평가_격차율_최고', value: '결정가격_격차율_최고'},
...
function MC_302_비준가격_Change_1_본건() {
    ...
    SetValue('Edit_비준_소재지_본건', (GetValue('Edit_입력표_소재지1') || '') + " " + (GetValue('Edit_입력표_지번') || ''));
    ...
    if (GetValue('Combo_비준_사례구분') == '1') {
        MC_302_비준가격_Change_2_사례_가격();
    } else if (GetValue('Combo_비준_사례구분') == '2') {
...
function MC_303_비준가격_추가시_Grid() {
    ...
    GetComponent('DBGrid_비준가격').FindIndex('감정가_비준_현재행', (row) => {
        ...
        if (row['사례구분'] == GetValue('Combo_비준_사례구분') && row['사례번호'] == GetValue('Combo_비준_사례번호')) {
            return true;
        }
...
function MC_322_수익가격_Change_1_본건() {
    ...
    SetValue('수익_Edit_소재지_본건', (GetValue('Edit_입력표_소재지1') || '') + " " + (GetValue('Edit_입력표_지번') || ''));
...
function MC_322_수익가격_Change_3_사례_지수값셋팅() {
    GetZoonData('Q_00_위치별').FindIndex('감정가_수익_위치별_IDX', (row) => {
    ...
    GetZoonData('Q_00_접근조건').FindIndex('감정가_수익_접근조건_IDX', (row) => {
...
function MC_401_평가액_최고최저_0() {
    ...
    if (GetNumber('00_RowIndex') < 0) {
        ...
        GetComponent('DBGrid_본건사례').GetRow(GetNumber('00_RowIndex'), [
            ...
            { key: '01_TNUM', value: '전용면적'},
        ]);
        SetValue('MkEdit_평가_본건_최저_단가', Round((GetNumber('MkEdit_평가_본건_최저_총액')/GetNumber('01_TNUM'))/1000, 0)*1000);
        SetValue('MkEdit_평가_본건_최고_단가', Round((GetNumber('MkEdit_평가_본건_최고_총액')/GetNumber('01_TNUM'))/1000, 0)*1000);
...
function MC_910_저장_입력표() {
    ...
    RunQuery('저장_20_낙찰가율보정표', {
        ...
        'PL_BID_AMT' : GetValue('MkEdit_보정표_예상낙찰가'),
        'BUS_APPRO_CODE' : GetValue('Combo_보정표_상가의접근성'),
        'BUS_APPRO_RATE' : GetValue('MkEdit_보정표_상가의접근성_적용율'),
        'BUS_FLOOR_LOC' : GetValue('Combo_보정표_상가의층별위치'),
        'BUS_FLOOR_RATE' : GetValue('MkEdit_보정표_상가의층별위치_적용율'),
        'BUS_COMM_DEV' : GetValue('Combo_보정표_상권의발달정도'),
        'BUS_COMM_RATE' : GetValue('MkEdit_보정표_상권의발달정도_적용율'),
        'BUS_ELAP_CODE' : GetValue('Combo_보정표_건축물경과년도'),
        'BUS_ELAP_RATE' : GetValue('MkEdit_보정표_건축물의경과년도_적용율'),
    }, 'POST');
    ...
    if (match(PageType.집합_상업용)) {
        ...
        RunQuery('저장_10_감정가_본건', {
            ...
            '저장_본건_운용이율' : GetValue('저장_본건_운용이율'),
            '저장_본건_총연간임대료' : GetValue('저장_본건_총연간임대료'),
            '저장_본건_단위당연간임대료' : GetValue('저장_본건_단위당연간임대료'),
        }, 'POST');
        ...
        RunQuery('저장_18_감정가_수익가격', {
            ...
            '저장_수익_도로조건_본건_적용율' : GetValue('저장_수익_도로조건_본건_적용율'),
            '저장_수익_도로조건_사례_적용율' : GetValue('저장_수익_도로조건_사례_적용율'),
