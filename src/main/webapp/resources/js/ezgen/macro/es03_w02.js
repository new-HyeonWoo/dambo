...
function MC_담보감정일자_초기화() {
    ...
    if (Month(Now()) < 10 ) {
        SetValue('Combo_까지_월', "0"+NumToStr(Month(Now())))
    }

    SetValue('00_TEMP', DateToStr(Now()))


    RunQuery('Q_DUAL_해당월일자_부터', {}, 'GET');


    RunQuery('Q_DUAL_해당월일자_까지', {}, 'GET');
    ...
    GetComponent('Combo_우편번호_시도').SetCurSel(0.0);
