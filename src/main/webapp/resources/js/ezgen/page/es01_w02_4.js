const pageEvent = {
    ...
    OnInitialize : function() {
        ...
        if (match(PageType.집합_아파트)) {
            ...
            GetComponent('DBGrid_가격사례_M').GetRowCount('가격사례_D_RowCount');
    ...
    OnAfter : function () {
        ...
        GetComponent('Picture72_배당자료수정').updateOptions({ isShow : GetValue('01_배당자료수정가능여부') == 'Y'});
        GetComponent('Picture_전감정사항비교표').updateOptions({ isShow : GetString('버튼_전감정사항비교표') == 'Y'});
        GetComponent('Picture60_유입검토보고서').updateOptions({ isShow : GetString('응찰_입력표활성여부') == "Y"});
