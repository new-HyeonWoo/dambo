...
<table class="table table-row-bordered border table-hover align-middle gs-5 gy-3 gx-5">
	...
	<tbody>
		...
		<tr>
			<td class="fs-10 text-gray-800 text-nowrap">{{#list}} {{지점_감정자_담당자}} {{/list}}</td>
			<td class="fs-10 text-gray-800 text-nowrap">{{#list}} {{지점_감정자_파트장}} {{/list}}</td>
			<td class="fs-10 text-gray-800 text-nowrap">{{#list}} {{지점_감정자_팀장}} {{/list}}</td>
			<td class="fs-10 text-gray-800 text-nowrap">{{#list}} {{지점_감정자_지점장}} {{/list}}</td>
			<td class="fs-10 text-gray-800 text-nowrap">{{#list}} {{본사_감정자_1차결재}} {{/list}}</td>
			<td class="fs-10 text-gray-800 text-nowrap">{{#list}} {{본사_감정자_2차결재}} {{/list}}</td>
			<td class="fs-10 text-gray-800 text-nowrap">{{#list}} {{본사_감정자_3차결재}} {{/list}}</td>
			<td class="fs-10 text-gray-800 text-nowrap">{{#list}} {{본사_감정자_관재파트장}} {{/list}}</td>
		</tr>
		<tr>
			<td class="fw-bolder fs-7 text-gray-800">3. 제1차예상입찰가</td>
			<td class="text-end fs-10 text-gray-800" colspan="4">{{#list}} {{#지점_제1차예상입찰가}} {{지점_제1차예상입찰가}} 원{{/지점_제1차예상입찰가}}{{/list}}</td>
			<td class="text-end fs-10 text-gray-800" colspan="4">{{#list}} {{#본사_제1차예상입찰가}} {{본사_제1차예상입찰가}} 원{{/본사_제1차예상입찰가}}{{/list}}</td>
		</tr>
		<tr>
			<td class="fw-bolder fs-7 text-gray-800">4. 예상낙찰가율</td>
			<td class="text-end fs-10 text-gray-800" colspan="4">{{#list}} {{#지점_예상낙찰가율}} {{지점_예상낙찰가율}} %{{/지점_예상낙찰가율}}{{/list}}</td>
			<td class="text-end fs-10 text-gray-800" colspan="4">{{#list}} {{#본사_예상낙찰가율}} {{본사_예상낙찰가율}} %{{/본사_예상낙찰가율}}{{/list}}</td>
		</tr>
		<tr>
			<td class="fw-bolder fs-7 text-gray-800">5. 예상낙찰가</td>
			<td class="text-end fs-10 text-gray-800" colspan="4">{{#list}} {{#지점_예상낙찰가}} {{지점_예상낙찰가}} 원{{/지점_예상낙찰가}}{{/list}}</td>
			<td class="text-end fs-10 text-gray-800" colspan="4">{{#list}} {{#본사_예상낙찰가}} {{본사_예상낙찰가}} 원{{/본사_예상낙찰가}}{{/list}}</td>
		</tr>
		<tr>
			<td class="fw-bolder fs-7 text-gray-800">6. 담보여력</td>
			<td class="text-end fs-10 text-gray-800" colspan="4">{{#list}} {{#지점_담보여력}} {{지점_담보여력}} 원{{/지점_담보여력}}{{/list}}</td>
			<td class="text-end fs-10 text-gray-800" colspan="4">{{#list}} {{#본사_담보여력}} {{본사_담보여력}} 원{{/본사_담보여력}}{{/list}}</td>
		</tr>
		<tr>
			<td class="fw-bolder fs-7 text-gray-800">7. 당사채권금액</td>
			<td class="text-end fs-10 text-gray-800" colspan="4">{{#list}} {{#지점_당사채권금액}} {{지점_당사채권금액}} 원{{/지점_당사채권금액}}{{/list}}</td>
			<td class="text-end fs-10 text-gray-800" colspan="4">{{#list}} {{#본사_당사채권금액}} {{본사_당사채권금액}} 원{{/본사_당사채권금액}} {{/list}}</td>
		</tr>
		<tr>
			<td class="fw-bolder fs-7 text-gray-800">8. 당사순익 및 등기권리</td>
			<td class="text-end fs-10 text-gray-800" colspan="4">{{#list}} {{지점_당사순위및등기권리}} {{/list}}</td>
			<td class="text-end fs-10 text-gray-800" colspan="4">{{#list}} {{본사_당사순위및등기권리}} {{/list}}</td>
		</tr>
	</tbody>
</table>
</script>
...
<table class="table table-row-bordered border table-hover gs-5 gy-3 gx-5">
	<thead class="bg-lighten align-middle">
		...
		{{/prt41}}
		{{^prt41}}
			<tr class="fw-normal text-gray-800">
				<th></th>
				<th></th>
				<th colspan="2"></th>
				<th></th>
...
<table class="table table-row-bordered border gs-5 gy-3 gx-5">
	<thead class="align-middle">
		<tr class="fw-bolder fs-7 text-gray-800">
			<th>응찰여부의 결정</th>
			<th class=" fs-10">{{#list}}{{수지8_응찰여부결정}}{{/list}}</th>
		</tr>
	</thead>
</table>
</script>
...
<table class="table table-row-bordered border table-hover gs-5 gy-3 gx-5">
	...
	<tfoot>
		...
		<tr>
			...
			<td></td>
			<td></td>
			<td class="text-end fs-10">{{#토지_면적}} {{토지_면적}} m<sup>2</sup>{{/토지_면적}}</td>
			<td class="text-end fs-10">{{#토지_면적_평}} {{토지_면적_평}} 평{{/토지_면적_평}}</td>
			<td class="text-end fs-10">{{#토지_개별공시지가_단가}} {{토지_개별공시지가_단가}} 원/m<sup>2</sup>{{/토지_개별공시지가_단가}}</td>
			<td class="text-end fs-10">{{#토지_개별공시지가_단가_평}} {{토지_개별공시지가_단가_평}} 원/평{{/토지_개별공시지가_단가_평}}</td>
			<td class="text-end fs-10">{{#토지_개별공시지가}} {{토지_개별공시지가}} 원{{/토지_개별공시지가}} </td>
		</tr>
		{{/prt11_d_tot}}
		<tr>
...
<table class="table table-row-bordered border gs-5 gy-3 gx-5">
	...
	<tbody>
		<tr>
			...
			<td>최저가</td>
			<td class="text-end fs-10">{{#list}} {{#MIN_AMT}} {{MIN_AMT}} 원{{/MIN_AMT}} {{/list}}</td>
			<td class="text-end fs-10">{{#list}} {{#MIN_DAN}} {{MIN_DAN}} 원/m<sup>2</sup> {{/MIN_DAN}}{{/list}}</td>
			<td class="text-end fs-10">{{#list}} {{#MIN_DAN_PY}} {{MIN_DAN_PY}} 원/평{{/MIN_DAN_PY}} {{/list}}</td>
		</tr>
		<tr>
			<td>최고가</td>
			<td class="text-end fs-10">{{#list}} {{#MAX_AMT}} {{MAX_AMT}} 원{{/MAX_AMT}} {{/list}}</td>
			<td class="text-end fs-10">{{#list}} {{#MAX_DAN}} {{MAX_DAN}} 원/m<sup>2</sup>{{/MAX_DAN}} {{/list}}</td>
			<td class="text-end fs-10">{{#list}} {{#MAX_DAN_PY}} {{MAX_DAN_PY}}원/평 {{/MAX_DAN_PY}} {{/list}}</td>
		</tr>
		<tr>
			<td colspan="2" class="fw-bolder fs-7 text-gray-800">2. 실거래가격 (등기부상 실거래가격)</td>
			<td class="text-end fs-10">{{#prt12}} {{#등기부등본상의시세_총액}} {{등기부등본상의시세_총액}} 원{{/등기부등본상의시세_총액}} {{/prt12}}</td>
			<td class="text-end fs-10">{{#prt12}} {{#등기부등본상의시세 }} {{등기부등본상의시세}} 원/m<sup>2</sup>{{/등기부등본상의시세 }} {{/prt12}}</td>
			<td class="text-end fs-10">{{#prt12}} {{#등기부등본상의시세_단가}} {{등기부등본상의시세_단가}} 원/평{{/등기부등본상의시세_단가}} {{/prt12}}</td>
		</tr>
		<tr>
			<td colspan="2" class="fw-bolder fs-7 text-gray-800">3. 개별주택가격</td>
			<td class="text-end fs-10">{{#prt12}} {{#개별주택가격_총액}} {{개별주택가격_총액}} 원{{/개별주택가격_총액}} {{/prt12}}</td>
			<td class="text-end fs-10">{{#prt12}} {{#개별주택가격_단가}} {{개별주택가격_단가}} 원/m<sup>2</sup>{{/개별주택가격_단가}} {{/prt12}}</td>
			<td class="text-end fs-10">{{#prt12}} {{#개별주택가격_단가_평}} {{개별주택가격_단가_평}} 원/평{{/개별주택가격_단가_평}} {{/prt12}}</td>
		</tr>
		<tr>
			<td rowspan="3" class="fw-bolder fs-7 text-gray-800">4.평가가격</td>
			<td>토지</td>
			<td class="text-end fs-10">{{#prt22}} {{평가가격}} 원{{/prt22}}</td>
			<td class="text-end fs-10">{{#prt22}} {{평가가격_단가}} 원/m<sup>2</sup>{{/prt22}}</td>
			<td class="text-end fs-10">{{#prt22}} {{평가가격_단가_평}} 원/평{{/prt22}}</td>
		</tr>
		<tr>
			<td>건물</td>
			<td class="text-end fs-10">{{#prt21}} {{평가가격}} 원{{/prt21}}</td>
			<td class="text-end fs-10">{{#prt21}} {{평가가격_단가}} 원/m<sup>2</sup>{{/prt21}}</td>
			<td class="text-end fs-10">{{#prt21}} {{평가가격_단가_평}} 원/평{{/prt21}}</td>
		</tr>
		<tr>
			<td></td>
			<td class="text-end fs-10">{{#list_tot}} {{list_tot}} 원{{/list_tot}}</td>
			<td></td>
			<td></td>
		</tr>
	</tbody>
	...
	<div class="col-xxl-12">
		<div class="card" id="print">
			...
			<div class="card-body">
				...
				<h5 style="page-break-before:always">Ⅳ. 전감정서 개요(담보설정시점의 감정내용 요약)</h5>
				...
				<h5>Ⅷ. 취득, 보유, 처분에 따른 비용과 수익 및 예상수지</h5>
...
<script type="text/javascript" src="<c:url value='/resources/js/es83/es83_w03/prt_00.js'/>"></script>
<script type="text/javascript" src="<c:url value='/resources/js/es83/es83_w03/prt_02.js'/>"></script>
<script type="text/javascript" src="<c:url value='/resources/js/es83/es83_w03/prt_03.js'/>"></script>
<script type="text/javascript" src="<c:url value='/resources/js/es83/es83_w03/prt_11.js'/>"></script>
<script type="text/javascript" src="<c:url value='/resources/js/es83/es83_w03/prt_12.js'/>"></script>
<script type="text/javascript" src="<c:url value='/resources/js/es83/es83_w03/prt_13.js'/>"></script>
<script type="text/javascript" src="<c:url value='/resources/js/es83/es83_w03/prt_21.js'/>"></script>
<script type="text/javascript" src="<c:url value='/resources/js/es83/es83_w03/prt_22.js'/>"></script>
<script type="text/javascript" src="<c:url value='/resources/js/es83/es83_w03/prt_41.js'/>"></script>
<script type="text/javascript" src="<c:url value='/resources/js/es83/es83_w03/prt_40.js'/>"></script>
<script type="text/javascript" src="<c:url value='/resources/js/es83/es83_w03/prt_42.js'/>"></script>
</body>
</html>