...
<table class="table table-row-bordered border table-hover align-middle gs-5 gy-3 gx-5">
	...
	<tbody>
		...
		<tr>
			<td class="fs-10 text-gray-800">{{#list}} {{지점_감정자_담당자}} {{/list}}</td>
			<td class="fs-10 text-gray-800">{{#list}} {{지점_감정자_파트장}} {{/list}}</td>
			<td class="fs-10 text-gray-800">{{#list}} {{지점_감정자_팀장}} {{/list}}</td>
			<td class="fs-10 text-gray-800">{{#list}} {{지점_감정자_지점장}} {{/list}}</td>
			<td class="fs-10 text-gray-800">{{#list}} {{본사_감정자_1차결재}} {{/list}}</td>
			<td class="fs-10 text-gray-800">{{#list}} {{본사_감정자_2차결재}} {{/list}}</td>
			<td class="fs-10 text-gray-800">{{#list}} {{본사_감정자_3차결재}} {{/list}}</td>
			<td class="fs-10 text-gray-800">{{#list}} {{본사_감정자_관재파트장}} {{/list}}</td>
		</tr>
		<tr>
...
<table class="table table-row-bordered border gs-5 gy-3 gx-5">
	<thead class="align-middle">
		<tr class="fs-7 text-gray-800">
			...
			<th class="fs-10">{{#list}} {{수지9_권리내역_존재여부}} {{/list}}</th>
			<th>내역합계</th>
			<th class="fs-10">{{#list}} {{수지9_권리내역_건수}} {{/list}}</th>
		</tr>
		<tr class="fs-7 text-gray-800">
			<th>낙찰인이 인수하는 선순위 금액</th>
			<th class="fs-10">{{#list}} {{#수지1_경락후_부담비용합계}} {{수지1_경락후_부담비용합계}} 원{{/수지1_경락후_부담비용합계}} {{/list}}</th>
			<th rowspan="4">총취득원가 합계</th>
			<th class="fs-10" rowspan="4">{{#list}} {{#수지4_총취득원가합계}} {{수지4_총취득원가합계}} 원{{/수지4_총취득원가합계}} {{/list}}</th>
		</tr>
		<tr class="fs-7 text-gray-800">
			<th class="fw-bolder" rowspan="3">취득</th>
			<th>입찰예정가</th>
			<th class="fs-10">{{#list}} {{#내용_입찰예정가_금액}} {{내용_입찰예정가_금액}} 원{{/내용_입찰예정가_금액}} {{/list}}</th>
		</tr>
		<tr class="fs-7 text-gray-800">
			<th>제세금</th>
			<th class="fs-10">{{#list}} {{#수지4_제세금}} {{수지4_제세금}} 원{{/수지4_제세금}}{{/list}}</th>
		</tr>
		<tr class="fs-7 text-gray-800">
			<th>기타비율</th>
			<th class="fs-10">{{#list}} {{#수지4_기타비용}} {{수지4_기타비용}} 원{{/수지4_기타비용}} {{/list}}</th>
		</tr>
		<tr class="fs-7 text-gray-800">
			<th class="fw-bolder" rowspan="2">보유</th>
			<th>수익</th>
			<th class="fs-10">{{#list}} {{#수지5_보유수익합계}} {{수지5_보유수익합계}} 원{{/수지5_보유수익합계}} {{/list}}</th>
		</tr>
		<tr class="fs-7 text-gray-800">
			<th>비용</th>
			<th class="fs-10">{{#list}} {{#수지6_보유비용합계}} {{수지6_보유비용합계}}  원{{/수지6_보유비용합계}} {{/list}}</th>
		</tr>
		<tr class="fs-7 text-gray-800">
			<th class="fw-bolder" rowspan="2">예상매각금액</th>
			<th>추정 예상 매각금액</th>
			<th class="fs-10">{{#list}} {{#수지3_추정예상매각금액}} {{수지3_추정예상매각금액}} 원{{/수지3_추정예상매각금액}} {{/list}}</th>
		</tr>
		<tr class="fs-7 text-gray-800">
			<th>매각확신금액</th>
			<th class="fs-10">{{#list}} {{#수지3_매각확신금액}} {{수지3_매각확신금액}} 원{{/수지3_매각확신금액}} {{/list}}</th>
		</tr>
		<tr class="fs-7 text-gray-800">
			<th class="fw-bolder">처분</th>
			<th>비용</th>
			<th class="fs-10">{{#list}} {{#수지7_처분비용합계}} {{수지7_처분비용합계}} 원{{/수지7_처분비용합계}} {{/list}}</th>
		</tr>
		<tr class="fs-7 text-gray-800">
			<th class="fw-bolder" rowspan="3">2. 항목별 집계</th>
			<th class="fw-bolder" colspan="2">비용의 합계</th>
			<th class="fs-10">{{#list}} {{#수지8_각항목별합계_비용합계}} {{수지8_각항목별합계_비용합계}} 원{{/수지8_각항목별합계_비용합계}} {{/list}}</th>
		</tr>
		<tr class="fs-7 text-gray-800">
			<th class="fw-bolder" colspan="2">수익의 합계</th>
			<th class="fs-10">{{#list}} {{#수지8_각항목별합계_수익합계}} {{수지8_각항목별합계_수익합계}} 원{{/수지8_각항목별합계_수익합계}} {{/list}}</th>
		</tr>
		<tr class="fs-7 text-gray-800">
			<th class="fw-bolder" colspan="2">예상손익</th>
			<th class="fs-10">{{#list}} {{#수지8_각항목별합계_예상손익}} {{수지8_각항목별합계_예상손익}} 원{{/수지8_각항목별합계_예상손익}}{{/list}}</th>
		</tr>
		<tr class="fs-7 text-gray-800">
			<th class="fw-bolder" colspan="3">3. 최종적인 전체손익</th>
			<th class="fs-10" >{{#list}} {{#수지8_최종적인전체손익}} {{수지8_최종적인전체손익}} 원{{/수지8_최종적인전체손익}} {{/list}}</th>
		</tr>
	</thead>
</table>
</script>
...
<body>
	<div class="col-xxl-12">
		<div class="card" id="print">
			<div id ="prt_00_title" class ="text-center md-5 mt-5"></div>
...
<script type="text/javascript" src="<c:url value='/resources/js/es83/es83_w02/prt_00.js'/>"></script>
<script type="text/javascript" src="<c:url value='/resources/js/es83/es83_w02/prt_01.js'/>"></script>
<script type="text/javascript" src="<c:url value='/resources/js/es83/es83_w02/prt_02.js'/>"></script>
<script type="text/javascript" src="<c:url value='/resources/js/es83/es83_w02/prt_03.js'/>"></script>
<script type="text/javascript" src="<c:url value='/resources/js/es83/es83_w02/prt_11.js'/>"></script>
<script type="text/javascript" src="<c:url value='/resources/js/es83/es83_w02/prt_41.js'/>"></script>
<script type="text/javascript" src="<c:url value='/resources/js/es83/es83_w02/prt_40.js'/>"></script>
<script type="text/javascript" src="<c:url value='/resources/js/es83/es83_w02/prt_42.js'/>"></script>
</body>
</html>