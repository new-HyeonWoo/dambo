...
<head>
    ...
    <script type="text/javaScript" language="javascript" defer="defer">
        ...
        function fn_Search_Button(){
			document.listForm.action = "<c:url value='/es02/es02_w06.do'/>";
        ...
        function fn_Search_New(){
        	...
        	var scroll="AUTO";
        	var target='appnew';
        	url ="<c:url value='/es02/es02_w06_new.do'/>";
		...
		function fn_Search_Manager_Button() {
			window.open('/es02/es02_w01.do');
		}

		function fn_Delete_Button(){
			let delete_id = '';
			let delete_boolean = false;
			for(let i =0; i < document.getElementById('dataTable_tobody').children.length; i++){
				if(document.getElementById('dataTable_tobody').children[i].classList.contains('table-active') == true){
					delete_id = document.getElementById('dataTable_tobody').children[i].id;
					delete_boolean = true;
					break;
				}
			}
			...
			const delete_secCode = document.getElementById(delete_id+'_secCode').value;
			const delete_yyyy = document.getElementById(delete_id+'_yyyy').value;
			const delete_seq = document.getElementById(delete_id+'_seq').value;
			const delete_procDiv = document.getElementById(delete_id+'_procDiv').value;
			const delete_bidUseYn = document.getElementById(delete_id+'_bidUseYn').value;

			if(delete_procDiv == '29'){
				alert('[검증완료]된 자료는 삭제할 수 없습니다');
				return;
			}	
		...
		function fn_reassessment_Button(){
			...
			$.ajax({
				...
				contentType:'application/json',
				data: JSON.stringify(param),
				url: "/es02/es02_w06_reassessment_number.do",
				success: function(json) {
					if(json.response == true){
		...
		<div class="toolbar" id="kt_toolbar">
			<div id="kt_toolbar_container" class="container-fluid d-flex flex-stack">
				<div data-kt-swapper="true" data-kt-swapper-mode="prepend" data-kt-swapper-parent="{default: '#kt_content_container', 'lg': '#kt_toolbar_container'}" class="page-title d-flex align-items-center flex-wrap me-3 mb-5 mb-lg-0">
					<h1 class="d-flex text-dark fw-bolder fs-3 align-items-center my-1">검증감정현황</h1>
					...
					<ul class="breadcrumb breadcrumb-separatorless fw-bold fs-7 my-1">
						...
						<li class="breadcrumb-item text-dark">검증감정현황</li>
					</ul>
				</div>
				<div class="d-flex align-items-center gap-2 gap-lg-3">
					<a href="javascript:fn_Delete_Button();" class="btn btn-sm btn-flex btn-jinro btn-active-jinro fw-bolder">
						<i class="fas fa-search"></i>
						삭제
					</a>
					<a href="javascript:fn_Search_Manager_Button();" class="btn btn-sm btn-flex btn-jinro btn-active-jinro fw-bolder">
						<i class="fas fa-search"></i>
						담당자별감정현황
					</a>
								...
								<table id="dataTable" class="table table-hover table-row-bordered gy-5 gs-7 text-center">
									<thead class="bg-secondary">
										<tr class="fw-bolder fs-6 text-gray-800  text-muted">
											...
											<th class="min-w-100px">지점담당자</th>
											<th class="min-w-100px">관재담당자</th>
										</tr>
									</thead>
									<tbody class="fs-7" id="dataTable_tobody" class="text-center text-nowrap">
						        		<c:forEach var="result" items="${sampleList}" varStatus="status">
					            			<tr id='${result.seq}' onclick="fn_tr_activate(`${result.seq}`)" ondblclick="fn_detail_page('${result.yyyy}','${result.seq}','${result.secCode}','${result.docKey}','${result.procDiv}','${result.liquorType}','${result.preYyyy}','${result.preSeq}', '${result.bidUseYn}')" >
					            				...
					            				<td class="text-center text-nowrap"><c:out value="${result.jumSabunName}"/></td>
					            				<td class="text-center text-nowrap"><c:out value="${result.manaSabunName}"/></td>
