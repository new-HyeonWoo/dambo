    ...
    <table class="table table-row-bordered table-rounded border table-hover gs-5 gy-3 gx-5">
        <thead class="align-top bg-secondary text-nowrap text-center">
            <tr class="fw-bolder fs-7 text-gray-800">
                ...
                <th>+</th>
                <th class="min-w-120px">이용상황</th>
                <th class="min-w-120px">전용면적 (㎡)</th>
                <th class="min-w-200px">해당층</th>
                <th class="min-w-100px">비준율</th>
                <th class="min-w-100px">위치</th>
                <th class="min-w-250px">용도지역</th>
...
<script id="DBGrid_가격사례_템플릿_ROW" type="text/x-handlebars-template">
    <tr>
        ...
        <td>{{ maskField name="건축년도" value=건축년도 index=index options='{"mask": "0000", "maskType": "숫자", "textType":"text-center"}' }}</td>
        <td>{{ noneField name="경과년도" value=경과년도 index=index options='{"mask": "-;2;#,###1", "maskType": "숫자", "textType":"text-end"}' }}</td>
        <td>{{ comboField name="구조" value=구조 index=index query='{"id" : "Q_00_구조", "text" : "CD_DESC", "value" : "CD_CODE"}' options='{}' }}</td>
        <td>{{ noneField name="내용년수" value=내용년수 index=index options='{"mask": "####", "maskType": "숫자", "textType":"text-end"}' }}</td>
        ...
        <td>{{ maskField name="조사금액" value=조사금액 index=index options='{"mask": "-;12;#,###1", "maskType": "숫자", "textType":"text-end"}' }}</td>
        <td>{{ noneField name="사례가격" value=사례가격 index=index options='{"mask": "-;12;#,###1", "maskType": "숫자", "textType":"text-end"}' }}</td>
...
<script id="DBGrid_경매사례_템플릿_ROW" type="text/x-handlebars-template">
    <tr class="">
        ...
        <td>{{ inputField name="소재지" value=소재지 index=index options='{"textType":"text-start"}' }}</td>
        <td>{{ buttonField name="주소팝업" value="주소팝업" index=index options='{ "iconType" : "search" }' }}</td>
        <td>{{ inputField name="이용상황" value=이용상황 index=index query='{"id" : "Q_DUAL_이용상황", "text" : "CD_DESC", "value" : "CD_CODE"}' options='{"textType":"text-start"}' }}</td>
        <td>{{ maskField name="전용면적" value=전용면적 index=index options='{"mask": "-;7;#,###1.00", "maskType": "숫자", "textType":"text-end"}' }} </td>
        <td>{{ comboField name="층수_코드" value=층수_코드 index=index query='{"id" : "Q_00_전유부분의위치", "text" : "CD_DESC", "value" : "CD_CODE"}' options='{}' }}</td>
        <td>{{ noneField name="층수_비준율" value=층수_비준율 index=index options='{"mask": "-;7;#,###1.00", "maskType": "숫자", "textType":"text-end"}' }}</td>
        <td>{{ comboField name="위치" value=위치 index=index query='{"id" : "Q_00_위치별", "text" : "CD_DESC", "value" : "CD_CODE"}' options='{}' }}</td>
        <td>{{ comboField name="용도지역" value=용도지역 index=index query='{"id" : "Q_00_용도지역", "text" : "CD_DESC", "value" : "CD_CODE"}' options='{}' }}</td>
        <td>{{ maskField name="건축년도" value=건축년도 index=index options='{"mask": "####", "maskType": "숫자", "textType":"text-center"}' }}</td>
        <td>{{ noneField name="경과년도" value=경과년도 index=index options='{"mask": "####", "maskType": "숫자", "textType":"text-end"}' }}</td>
        <td>{{ comboField name="구조" value=구조 index=index query='{"id" : "Q_00_구조", "text" : "CD_DESC", "value" : "CD_CODE"}' options='{}' }}</td>
        <td>{{ noneField name="내용년수" value=내용년수 index=index options='{"mask": "-;2;#,###1", "maskType": "숫자"}' }}</td>
        <td>{{ comboField name="도로조건" value=도로조건 index=index query='{"id" : "Q_00_도로조건", "text" : "CD_DESC", "value" : "CD_CODE"}' options='{}' }}</td>
        <td>{{ comboField name="접근조건" value=접근조건 index=index query='{"id" : "Q_00_접근조건", "text" : "CD_DESC", "value" : "CD_CODE"}' options='{}' }}</td>
        ...
        <td>{{ noneField name="낙찰가율" value=낙찰가율 index=index options='{"mask": "-;7;#,###1.00", "maskType": "숫자", "textType":"text-end"}' }}</td>
        <td class="hide">{{ inputField name="임시" value=임시 index=index options='{}' }}</td>
    </tr>
</script>
    ...
    <table class="table table-row-bordered table-rounded border table-hover gs-5 gy-3 gx-5">
        <thead class="align-top bg-secondary text-nowrap">
            <tr class="fw-bolder fs-7 text-gray-800 text-center align-middle">
                ...
                <th class="hide">위치</th>
                <th rowspan="2">선택</th>
                <th class="min-w-100px" rowspan="2">해당층</th>
                <th class="min-w-200px" rowspan="2">층수</th>
                <th class="min-w-80px" rowspan="2">비준율</th>
                <th class="min-w-80px" rowspan="2">전용면적<br>(㎡)</th>
                <th colspan="3">시세조사내역</th>
                <th class="min-w-120px" rowspan="2">m<sup>2</sup>당 단가</th>
                <th colspan="2">임대내용</th>
                <th class="min-w-80px" rowspan="2">운용<br>이율</th>
                <th class="min-w-120px" rowspan="2">총연간<br>임대료</th>
                <th class="min-w-120px" rowspan="2">단위당<br>연감임대료</th>
            </tr>
            <tr class="fw-bolder fs-7 text-gray-800 text-center">
                <th class="min-w-130px">최저가격</th>
                <th class="min-w-130px">최고가격</th>
                <th class="min-w-130px">평균가격</th>
                <th class="min-w-130px">보증금</th>
                <th class="min-w-130px">임대료(월)</th>
            </tr>
        </thead>    
        <tbody>
            {{#each .}}
...
<script id="DBGrid_본건사례_템플릿_ROW" type="text/x-handlebars-template">
    <tr class="">
        ...
        <td>{{ checkField name="선택여부" value=선택여부 index=index options='{ "on" : "Y", "off" : "N", "isGroup" : true }' }}</td>
        <td>{{ noneField name="해당층" value=해당층 index=index options='{"textType" : "text-center"}' }}</td>
        <td>{{ comboField name="층수" value=층수 index=index query='{"id": "Q_00_전유부분의위치", "text": "CD_DESC", "value": "CD_CODE"}' options='{}' }}</td>
        <td>{{ noneField name="층수_비준율" value=층수_비준율 index=index options='{"mask": "-;7;#,###1.00", "maskType": "숫자"}' }}</td>
        <td>{{ maskField name="전용면적" value=전용면적 index=index options='{"mask": "-;7;#,###1.00", "maskType": "숫자"}' }}</td>
        <td>{{ maskField name="최저가" value=최저가 index=index options='{"mask": "-;12;#,###1", "maskType": "숫자"}' }}</td>
        <td>{{ maskField name="최고가" value=최고가 index=index options='{"mask": "-;12;#,###1", "maskType": "숫자"}' }}</td>
        <td>{{ noneField name="평균가격" value=평균가격 index=index options='{"mask": "-;12;#,###1", "maskType": "숫자"}' }}</td>
        <td>{{ noneField name="M2당단가" value=M2당단가 index=index options='{"mask": "-;12;#,###1", "maskType": "숫자"}' }}</td>
        <td>{{ maskField name="보증금" value=보증금 index=index options='{"mask": "-;12;#,###1", "maskType": "숫자"}' }}</td>
        <td>{{ maskField name="월임대료" value=월임대료 index=index options='{"mask": "-;12;#,###1", "maskType": "숫자"}' }}</td>
        <td><div class="row"><div class="col-8">{{ noneField name="운용이율" value=운용이율 index=index options='{"mask": "-;7;#,###1", "maskType": "숫자"}' }}</div><div class="col-4">%</div></div></td>
        <td>{{ noneField name="총연간임대료" value=총연간임대료 index=index options='{"mask": "-;12;#,###1", "maskType": "숫자"}' }}</td>
        <td>{{ noneField name="단위당연간임대료" value=단위당연간임대료 index=index options='{"mask": "-;12;#,###1", "maskType": "숫자"}' }}</td>
    </tr>
</script>
    ...
    <table class="table table-row-bordered table-rounded border table-hover gs-5 gy-3 gx-5">
        <thead class="align-top bg-secondary text-nowrap text-center">
            <tr class="fw-bolder fs-7 text-gray-800">
                ...
                <th class="hide">가격_본건_일자</th>
                <th class="hide">가격_본견_적용율</th>
                <th class="hide">가격_사례_적용율</th>
                <th class="hide">가격_비교치</th>
                <th class="hide">전용면적_사례</th>
                <th class="hide">사례가격</th>
...
<script id="DBGrid_비준가격_템플릿_ROW" type="text/x-handlebars-template">
    <tr>
        <td class="hide">{{ noneField name="년도" value=년도 index=index options='{}' }}</td>
        <td class="hide">{{ noneField name="일련번호" value=일련번호 index=index options='{}' }}</td>
        <td class="hide">{{ noneField name="전용면적" value=일련번호 index=index options='{}' }}</td>
        <td class="hide">{{ noneField name="가격_본건_일자" value=가격_본건_일자 index=index options='{}' }}</td>
        <td class="hide">{{ noneField name="가격_본견_적용율" value=가격_본견_적용율 index=index options='{}' }}</td>
        <td class="hide">{{ noneField name="가격_사례_적용율" value=가격_사례_적용율 index=index options='{}' }}</td>
        <td class="hide">{{ noneField name="가격_비교치" value=가격_비교치 index=index options='{}' }}</td>
        <td class="hide">{{ noneField name="전용면적_사례" value=가격_비교치 index=index options='{}' }}</td>
        <td class="hide">{{ noneField name="사례가격" value=사례가격 index=index options='{}' }}</td>
        ...
        <td class="hide">{{ noneField name="진가율비교_비교치" value=진가율비교_비교치 index=index options='{}' }}</td>
        <td class="hide">{{ noneField name="도로조건_비교치" value=도로조건_비교치 index=index options='{}' }}</td>
        <td class="hide">{{ noneField name="접근조건_본건_적용율" value=접근조건_본건_적용율 index=index options='{}' }}</td>
        ...
        <td class="hide">{{ noneField name="기타조건_사례_내용" value=기타조건_사례_내용 index=index options='{}' }}</td>
        ...
        <td class="hide">{{ comboField name="사례구분" value=사례구분 index=index query='{"id": "Q_DUAL_비준_사례구분", "text": "CD_DESC", "value": "CD_CODE"}' options='{"mask": "####", "maskType": "문자"}' }}</td>
        <td class="hide">{{ noneField name="사례번호" value=사례번호 index=index options='{}' }}</td>
        <td class="hide">{{ noneField name="산정단가" value=사례번호 index=index options='{}' }}</td>
        ...
        <td>{{ noneField name="요인합계" value=요인합계 index=index options='{"mask": "-;12;#,###1.00", "maskType": "숫자"}' }}</td>
        <td>{{ noneField name="가격_사례_일자" value=가격_사례_일자 index=index options='{"mask": "yyyy-mm-dd", "maskType": "날짜"}' }}</td>
        <td>{{ noneField name="적용단가" value=적용단가 index=index options='{"mask": "-;12;#,###1", "maskType": "숫자"}' }}</td>
    </tr>
</script>
...
<script id="DBGrid_비준가격집계표_템플릿_ROW" type="text/x-handlebars-template">
    <tr>
        ...
        <td class="hide">{{ noneField name="소재지" value=소재지_비교치 index=index options='{}' }}</td>
        <td class="hide">{{ noneField name="사례가격" value=사례가격 index=index options='{}' }} </td>
        <td class="hide">{{ noneField name="사례번호" value=사례번호 index=index options='{}' }} </td>
        <td>{{ noneField name="사례구분명" value=사례구분명 index=index options='{}' }}</td>
        <td>{{ noneField name="사례번호명" value=사례번호명 index=index options='{}' }}</td>
        <td>{{ noneField name="소재지" value=소재지 index=index options='{}' }} </td>
        <td>{{ noneField name="가격_비교치" value=가격_비교치 index=index options='{"mask": "-;12;#,###1.000", "maskType": "숫자"}' }}</td>
        <td>{{ noneField name="층별비교_비교치" value=층별비교_비교치 index=index options='{"mask": "-;7;#,###1.00", "maskType": "숫자"}' }}</td>
        <td>{{ noneField name="위치별비교_비교치" value=위치별비교_비교치 index=index options='{"mask": "-;7;#,###1.00", "maskType": "숫자"}' }}</td>
        <td>{{ noneField name="잔가율비교_비교치" value=잔가율비교_비교치 index=index options='{"mask": "-;7;#,###1.00", "maskType": "숫자"}' }}</td>
        <td>{{ noneField name="도로조건_비교치" value=도로조건_비교치 index=index options='{"mask": "-;7;#,###1.00", "maskType": "숫자"}' }}</td>
        <td>{{ noneField name="접근조건_비교치" value=접근조건_비교치 index=index options='{"mask": "-;7;#,###1.00", "maskType": "숫자"}' }}</td>
        <td>{{ noneField name="기타조건_비교치" value=기타조건_비교치 index=index options='{"mask": "-;7;#,###1.00", "maskType": "숫자"}' }}</td>
        <td>{{ noneField name="요인합계" value=요인합계 index=index options='{"mask": "-;7;#,###1.00", "maskType": "숫자"}' }}</td>
        <td>{{ noneField name="산정단가" value=산정단가 index=index options='{"mask": "-;12;#,###1", "maskType": "숫자"}' }}</td>
        <td>{{ noneField name="적용단가" value=적용단가 index=index options='{"mask": "-;12;#,###1", "maskType": "숫자"}' }}</td>
        <td>{{ noneField name="환산금액" value=환산금액 index=index options='{"mask": "-;12;#,###1", "maskType": "숫자"}' }}</td>
    </tr>
</script>
...
<script id="DBGrid_응찰_기일내역_템플릿_ROW" type="text/x-handlebars-template">
    <tr>
        ...
        <td>{{ maskField name="금액" value=금액 index=index options='{"mask": "-;12;#,###1", "maskType": "숫자"}' }}</td>
    ...
    <table class="table table-row-bordered table-rounded border table-hover gs-5 gy-3 gx-5">
        <thead class="align-top bg-secondary text-nowrap text-center">
            <tr class="fw-bolder fs-7 text-gray-800">
                ...
                <th>각구의 일련번호</th>
                <th>1동의 건물 내에서의 주거용 부분인 각구의 위치</th>
                <th>1구의 건물내에 있는 주거용방의 총수</th>
                <th>확정일자 보유여부 </th>
                <th>가임대보증금을 적용한 방의 총수</th>
                <th>1구의 건물의 실제 임대차보증금</th>
                <th>최종 적용가임대 보증금 (최우선변제 소액보증금)</th>
                <th>확정일자부 주택임대차 보증금</th>
...
<script id="DBGrid_배당표_주택임대차보증금_계산_템플릿_ROW" type="text/x-handlebars-template">
    <tr>
        ...
        <td>{{ maskField name="RESI_RM_CNT" value=RESI_RM_CNT index=index options='{}' }} </td>
        <td>{{ checkField name="FXDATE_YN" value=FXDATE_YN index=index options='{ "on" : "Y", "off" : "N"}' }} </td>
        <td>{{ maskField name="POSS_A_RM_CNT" value=POSS_A_RM_CNT index=index options='{"mask": "-;12;#,###1", "maskType": "숫자"}' }} </td>
        <td>{{ maskField name="LEASE_AMT" value=LEASE_AMT index=index options='{"mask": "-;12;#,###1", "maskType": "숫자"}' }} </td>
        <td>{{ noneField name="REP_BE_SEC_AMT" value=REP_BE_SEC_AMT index=index options='{}' }} </td>
        <td>{{ noneField name="FXLEA_AMT" value=FXLEA_AMT index=index options='{"mask": "-;12;#,###1", "maskType": "숫자"}' }} </td>
        <td>{{ noneField name="FXLEA_NO_AMT" value=FXLEA_NO_AMT index=index options='{"mask": "-;12;#,###1", "maskType": "숫자"}' }} </td>
    </tr>
</script>
    ...
    <table class="table table-row-bordered table-rounded border table-hover gs-5 gy-3 gx-5">
        <thead class="align-top bg-secondary text-nowrap text-center">
            <tr class="fw-bolder fs-7 text-gray-800">
                ...
                <th class="hide">REP_AF_SEC_AMT</th>
                <th>각구의<br>일련번호</th>
                <th>1동의 건물 내에서의<br>상업용 부분의<br>위치 및 면적</th>
                <th>가임대<br>적용여부</th>
                <th>임차인의 상호와 성명<br>(사업자등록 기준)</th>
                <th>상가임대차보증금</th>
                <th>확정일자<br>보유여부</th>
                <th>월세</th>
                <th>1구의 상가건물의<br>실제 임대차보증금<br>(보증금 + 환산보증금)</th>
                <th>최종적으로 적용되는<br>상가 가임대 보증금<br>(최우선변제 보증금)</th>
                <th>확정일자부<br>상가임대차 보증금</th>
                <th>확정일자부 없는<br>상가임대차 보증금</th>
            </tr>
        </thead>    
        <tbody>
            {{#each .}}
...
<script id="DBGrid_배당표_상가건물_임대차보증금_계산_템플릿_ROW" type="text/x-handlebars-template">
    <tr>
        ...
        <td>{{ maskField name="STO_LSEC_AMT" value=STO_LSEC_AMT index=index options='{"mask": "-;12;#,###1", "maskType": "숫자"}' }} </td>
        <td>{{ checkField name="FXDATE_YN" value=FXDATE_YN index=index options='{ "on" : "Y", "off" : "N"}' }} </td>
        <td>{{ maskField name="MRENT_AMT" value=MRENT_AMT index=index options='{"mask": "-;12;#,###1", "maskType": "숫자"}' }} </td>
        <td>{{ noneField name="LEASE_AMT" value=LEASE_AMT index=index options='{"mask": "-;12;#,###1", "maskType": "숫자"}' }} </td>
        <td>{{ noneField name="REP_BE_SEC_AMT" value=REP_BE_SEC_AMT index=index options='{"mask": "-;12;#,###1", "maskType": "숫자"}' }} </td>
        <td>{{ noneField name="FXLEA_AMT" value=FXLEA_AMT index=index options='{"mask": "-;12;#,###1", "maskType": "숫자"}' }} </td>
        <td>{{ noneField name="FXLEA_NO_AMT" value=FXLEA_NO_AMT index=index options='{"mask": "-;12;#,###1", "maskType": "숫자"}' }} </td>
    </tr>
</script>
...
<script id="DBGrid_배당금_계산_템플릿_ROW" type="text/x-handlebars-template">
    <tr>
        ...
        <td>{{ comboField name="DISP_RANK" value=DISP_RANK index=index query='{"id": "Q_코드_순위", "text": "CD_DESC", "value": "CD_CODE"}' isEnable=IS_ENABLE options='{}' }} </td>
        ...
        <td>{{ noneField name="CRED_SELF_AMT" value=CRED_SELF_AMT index=index options='{"mask": "-;12;#,###1", "maskType": "숫자"}' }} </td>
        <td>{{ maskField name="CRED_COMB_AMT" value=CRED_COMB_AMT index=index options='{"mask": "-;12;#,###1", "maskType": "숫자"}' }} </td>
    </tr>
</script>
...
<script id="DBGrid_수익가격_템플릿_ROW" type="text/x-handlebars-template">
    <tr>
        ...
        <td>{{ noneField name="사례번호명" value=사례번호명 index=index options='{}' }} </td>
        <td>{{ noneField name="요인합계" value=요인합계 index=index options='{"mask": "-;7;#,###1.00", "maskType": "숫자"}' }}</td>
        <td>{{ noneField name="시점_사례_일자" value=시점_사례_일자 index=index options='{"mask": "yyyy-mm-dd", "maskType": "날짜"}' }} </td>
        <td>{{ noneField name="수익단가_평" value=수익단가_평 index=index options='{"mask": "-;7;#,###1.00", "maskType": "숫자"}' }} </td>
    </tr>
</script>
...
<script id="DBGrid_수익가격집계표_템플릿_ROW" type="text/x-handlebars-template">
    <tr>
        ...
        <td class="hide">{{ noneField name="사례구분" value=사례구분 index=index options='{}' }}</td>
        <td class="hide">{{ noneField name="사례번호" value=사례번호 index=index options='{}' }}</td>
        <td>{{ noneField name="사례구분명" value=사례구분명 index=index options='{"mask": "####", "maskType": "문자"}' }} </td>
        <td>{{ noneField name="사례번호명" value=사례번호명 index=index options='{}' }} </td>
        <td>{{ noneField name="소재지" value=소재지 index=index options='{}' }} </td>
        <td>{{ noneField name="단위당연간임대료_사례" value=단위당연간임대료_사례 index=index options='{"mask": "-;12;#,###1", "maskType": "숫자"}' }} </td>
        <td>{{ noneField name="시점수정_비교치" value=시점수정_비교치 index=index options='{"mask": "-;7;#,###1.00", "maskType": "숫자"}' }} </td>
        <td>{{ noneField name="층별비교_비교치" value=층별비교_비교치 index=index options='{"mask": "-;7;#,###1.00", "maskType": "숫자"}' }} </td>
        <td>{{ noneField name="위치별비교_비교치" value=위치별비교_비교치 index=index options='{"mask": "-;7;#,###1.00", "maskType": "숫자"}' }} </td>
        <td>{{ noneField name="잔가율비교_비교치" value=잔가율비교_비교치 index=index options='{"mask": "-;7;#,###1.00", "maskType": "숫자"}' }} </td>
        <td>{{ noneField name="도로조건_비교치" value=도로조건_비교치 index=index options='{"mask": "-;7;#,###1.00", "maskType": "숫자"}' }} </td>
        ...
        <td>{{ noneField name="기타조건_비교치" value=기타조건_비교치 index=index options='{"mask": "-;7;#,###1.00", "maskType": "숫자"}' }} </td>
        <td>{{ noneField name="요인합계" value=요인합계 index=index options='{"mask": "-;12;#,###1.00", "maskType": "숫자"}' }} </td>
        <td>{{ noneField name="적용단위당임료" value=적용단위당임료 index=index options='{"mask": "-;6;#,###1", "maskType": "숫자"}' }} </td>
        <td>{{ noneField name="환원이율" value=환원이율 index=index options='{"mask": "-;7;#,###1.00", "maskType": "숫자"}' }}</td>
        <td>{{ noneField name="수익단가" value=수익단가 index=index options='{"mask": "-;12;#,###1", "maskType": "숫자"}' }} </td>
        <td>{{ noneField name="환산금액" value=환산금액 index=index options='{"mask": "-;12;#,###1", "maskType": "숫자"}' }} </td>
    </tr>
</script>
    ...
    <table class="table table-row-bordered table-rounded border table-hover gs-5 gy-3 gx-5">
        <thead class="align-top bg-secondary text-nowrap text-center align-middle">
            <tr class="fw-bolder fs-7 text-gray-800">
                <th class="hide">년도</th>
                <th class="hide">일련번호</th>
                <th class="min-w-50px"rowspan="2">사례번호</th>
                <th class="min-w-250px"rowspan="2">소재지</th>
                <th rowspan="2">+</th>
                <th class="min-w-100px" rowspan="2">임대전용면적(㎡)</th>
                <th class="min-w-100px" rowspan="2">이용상황</th>
                <th class="min-w-200px" rowspan="2">해당층</th>
                <th class="min-w-100px" rowspan="2">비준율</th>
                <th class="min-w-100px" rowspan="2">위치 </th>
                <th class="min-w-200px" rowspan="2">용도지역</th>
                <th class="min-w-100px" rowspan="2">건축년도</th>
                <th class="min-w-100px" rowspan="2">경과년도</th>
                <th class="min-w-200px" rowspan="2">구조</th>
                <th class="min-w-100px" rowspan="2">내용년수</th>
                <th class="min-w-150px" rowspan="2">도로조건</th>
                <th class="min-w-100px" rowspan="2">접근조건</th>
                <th colspan="4">임대내역</th>
                <th class="min-w-150px" rowspan="2">단위당연간임대료(원/㎡)</th>
                <th class="hide">임시</th>
            </tr>
            <tr class="fw-bolder fs-7 text-gray-800">
                <th class="min-w-150px">보증금</th>
                <th class="min-w-150px">월임대료</th>
                <th class="min-w-100px">운용이율</th>
                <th class="min-w-150px">총연간 임대료</th>
            </tr>    
        </thead>    
        <tbody>
            {{#each .}}
...
<script id="DBGrid_임대사례_템플릿_ROW" type="text/x-handlebars-template">
    <tr>
        ...
        <td>{{ comboField name="위치" value=위치 index=index query='{"id": "Q_00_위치별", "text": "CD_DESC", "value": "CD_CODE"}' options='{}' }}  </td>
        <td>{{ comboField name="용도지역" value=용도지역 index=index query='{"id": "Q_00_용도지역", "text": "CD_DESC", "value": "CD_CODE"}' options='{}' }} </td>
        <td>{{ maskField name="건축년도" value=건축년도 index=index options='{"mask": "####", "maskType": "숫자", "textType": "text-end"}' }} </td>
        <td>{{ noneField name="경과년도" value=경과년도 index=index options='{"mask": "-;2;#,###1", "maskType": "숫자", "textType": "text-end"}' }} </td>
        <td>{{ comboField name="구조" value=구조 index=index query='{"id": "Q_00_구조", "text": "CD_DESC", "value": "CD_CODE"}' options='{}' }} </td>
        <td>{{ noneField name="내용년수" value=내용년수 index=index options='{"mask": "-;2;#,###1.00", "maskType": "숫자", "textType": "text-end"}' }} </td>
        <td>{{ comboField name="도로조건" value=도로조건 index=index query='{"id": "Q_00_도로조건", "text": "CD_DESC", "value": "CD_CODE"}' options='{}' }} </td>
        <td>{{ comboField name="접근조건" value=접근조건 index=index query='{"id": "Q_00_접근조건", "text": "CD_DESC", "value": "CD_CODE"}' options='{}' }} </td>
        <td>{{ maskField name="임대_보증금" value=임대_보증금 index=index options='{"mask": "-;12;#,###1", "maskType": "숫자", "textType": "text-end"}' }}  </td>
        <td>{{ maskField name="임대_월임대료" value=임대_월임대료 index=index options='{"mask": "-;12;#,###1", "maskType": "숫자", "textType": "text-end"}' }}  </td>
        <td><div class="row"><div class="col-8">{{ noneField name="임대_운용이율" value=임대_운용이율 index=index options='{"mask": "-;7;#,###1", "maskType": "숫자", "textType": "text-end"}' }}</div><div class="col-4">%</div></div></td>
        <td>{{ noneField name="임대_총연간임대료" value=임대_총연간임대료 index=index options='{"mask": "-;12;#,###1", "maskType": "숫자","textType":"text-end"}' }}  </td>
        <td>{{ noneField name="단위당연간임대료" value=단위당연간임대료 index=index options='{"mask": "-;12;#,###1", "maskType": "숫자", "textType":"text-end"}' }}  </td>
        <td class="hide">{{ noneField name="임시" value= 임시 index=index options='{}' }}</td>
    </tr>
</script>
